...
describe("Lesson 22: 高级安全主题", function () {
    ...
    beforeEach(async function () {
        ...
        vulnerablePool = await SimpleLendingPool.deploy(priceOracle.address);
        await vulnerablePool.deployed();

        const FlashLoanAttacker = await ethers.getContractFactory("FlashLoanAttacker");
        attacker = await FlashLoanAttacker.deploy(vulnerablePool.address, priceOracle.address);
        ...
        securePool = await SecureLendingPool.deploy(secureOracle.address);
        await securePool.deployed();
        ...
        await sigFixed.deployed();

        const SignatureReplayAttacker = await ethers.getContractFactory("SignatureReplayAttacker");
        sigAttacker = await SignatureReplayAttacker.deploy(sigVulnerable.address);
        await sigAttacker.deployed();
        ...
        await timeVulnerable.deployed();

        const TimeManipulationFixed = await ethers.getContractFactory("TimeManipulationFixed");
        timeFixed = await TimeManipulationFixed.deploy(86400, randomGenerator.address);
        await timeFixed.deployed();
        ...
        await comprehensiveVault.deployed();
    });
    ...
    describe("闪电贷攻击测试", function () {
        it("应该能正常存款和借款", async function () {
            ...
            expect(maxBorrow).to.equal(ethers.utils.parseEther("150.0")); // 100 * 100 * 150 / 10000
        ...
        it("应该演示价格操纵攻击", async function () {
            ...
            const manipulatedPrice = originalPrice.mul(10); // 提高 10 倍
            ...
            // 现在可以借更多
            const maxBorrow = await vulnerablePool.getBorrowingPower(user1.address);
            expect(maxBorrow).to.equal(ethers.utils.parseEther("1500.0")); // 100 * 1000 * 150 / 10000
        ...
        it("应该正确计算借款能力", async function () {
            ...
            expect(borrowingPower).to.equal(ethers.utils.parseEther("150.0"));
        });
        ...
        it("应该防止超额借款", async function () {
            ...
            const maxBorrow = ethers.utils.parseEther("150.0");
            const excessBorrow = ethers.utils.parseEther("200.0");
        ...
        it("应该能偿还借款", async function () {
            const depositAmount = ethers.utils.parseEther("100.0");
            const borrowAmount = ethers.utils.parseEther("50.0");
            const repayAmount = ethers.utils.parseEther("30.0");

            await vulnerablePool.connect(user1).deposit({ value: depositAmount });
            await vulnerablePool.connect(user1).borrow(borrowAmount);

            await vulnerablePool.connect(user1).repay({ value: repayAmount });
            expect(await vulnerablePool.borrows(user1.address)).to.equal(borrowAmount.sub(repayAmount));
        });
    });
    ...
    describe("签名重放攻击测试", function () {
        it("应该能使用签名取款", async function () {
            ...
            expect(await sigVulnerable.deposits(user1.address)).to.equal(depositAmount.sub(withdrawAmount));
        });
        ...
        it("应该能重放签名攻击（易受攻击的合约）", async function () {
            ...
            // 重放攻击（也能成功！）
            await sigVulnerable.withdrawWithSignature(withdrawAmount, signature);
            expect(await sigVulnerable.deposits(user1.address)).to.equal(depositAmount.sub(withdrawAmount.mul(2)));
        });
        ...
        it("修复后的合约应该防止重放攻击", async function () {
            ...
            const messageHash = ethers.utils.solidityKeccak256(
                ...
                [sigFixed.address, await sigFixed.chainId(), withdrawAmount, nonce]
            );
            ...
            expect(await sigFixed.deposits(user1.address)).to.equal(depositAmount.sub(withdrawAmount));
        ...
        it("应该正确更新 nonce", async function () {
            ...
            const messageHash1 = ethers.utils.solidityKeccak256(
                ["address", "uint256", "uint256", "uint256"],
                [sigFixed.address, await sigFixed.chainId(), withdrawAmount, nonce1]
            );
            const ethSignedHash1 = ethers.utils.solidityKeccak256(
                ["string", "bytes32"],
                ["\x19Ethereum Signed Message:\n32", messageHash1]
            );
            const signature1 = await user1.signMessage(ethers.utils.arrayify(ethSignedHash1));
    ...
    describe("综合安全金库测试", function () {
        it("应该能正常存款和取款", async function () {
            ...
            expect(await comprehensiveVault.deposits(user1.address)).to.equal(depositAmount.sub(withdrawAmount));
        });
        ...
        it("应该能使用签名取款并防止重放", async function () {
            ...
            const messageHash = ethers.utils.solidityKeccak256(
                ["address", "uint256", "uint256", "uint256"],
                [comprehensiveVault.address, await comprehensiveVault.chainId(), withdrawAmount, nonce]
            );
            const ethSignedHash = ethers.utils.solidityKeccak256(
                ["string", "bytes32"],
                ["\x19Ethereum Signed Message:\n32", messageHash]
        ...
        it("应该能暂停和恢复合约", async function () {
            ...
            // 暂停后不能存款
            await expect(
                comprehensiveVault.connect(user1).deposit({ value: ethers.utils.parseEther("1.0") })
            ...
            // 恢复后可以存款
            await comprehensiveVault.connect(user1).deposit({ value: ethers.utils.parseEther("1.0") });
            expect(await comprehensiveVault.deposits(user1.address)).to.equal(ethers.utils.parseEther("1.0"));
        });
        ...
        it("应该正确处理多个用户", async function () {
            await comprehensiveVault.connect(user1).deposit({ value: ethers.utils.parseEther("10.0") });
            await comprehensiveVault.connect(user2).deposit({ value: ethers.utils.parseEther("20.0") });
            await comprehensiveVault.connect(user3).deposit({ value: ethers.utils.parseEther("15.0") });

            expect(await comprehensiveVault.deposits(user1.address)).to.equal(ethers.utils.parseEther("10.0"));
            expect(await comprehensiveVault.deposits(user2.address)).to.equal(ethers.utils.parseEther("20.0"));
            expect(await comprehensiveVault.deposits(user3.address)).to.equal(ethers.utils.parseEther("15.0"));
        });
    });
    ...
    describe("安全对比测试", function () {
        it("应该对比易受攻击和修复后的实现", async function () {
            ...
            const nonce1 = 0;
            const messageHash1 = ethers.utils.solidityKeccak256(["uint256"], [amount]);
            const signature1 = await user1.signMessage(ethers.utils.arrayify(messageHash1));
            ...
            const messageHash2 = ethers.utils.solidityKeccak256(
                ["address", "uint256", "uint256", "uint256"],
                [sigFixed.address, await sigFixed.chainId(), amount, nonce2]
            );
            const ethSignedHash2 = ethers.utils.solidityKeccak256(
                ["string", "bytes32"],
                ["\x19Ethereum Signed Message:\n32", messageHash2]
            );
            const signature2 = await user1.signMessage(ethers.utils.arrayify(ethSignedHash2));
        ...
        it("应该展示不同安全措施的效果", async function () {
            ...
            // 测试防重入
            await comprehensiveVault.unpause();
            await comprehensiveVault.connect(user1).deposit({ value: ethers.utils.parseEther("5.0") });
            await comprehensiveVault.connect(user1).withdraw(ethers.utils.parseEther("2.0"));
            expect(await comprehensiveVault.deposits(user1.address)).to.equal(ethers.utils.parseEther("3.0"));
        });
    });
});
