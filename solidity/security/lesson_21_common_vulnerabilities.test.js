...
describe("Lesson 21: 常见漏洞攻击", function () {
    ...
    beforeEach(async function () {
        ...
        await overflowVulnerable.deployed();
        ...
        await frontRunningVulnerable.deployed();
        ...
        await frontRunningFixed.deployed();
        ...
        comprehensiveAttacker = await ComprehensiveAttacker.deploy(vulnerableVault.address);
        await comprehensiveAttacker.deployed();
    ...
    describe("重入攻击测试", function () {
        ...
        it("应该能成功进行重入攻击", async function () {
            ...
            expect(attackerBalanceAfter.sub(attackerBalanceBefore)).to.be.greaterThan(attackAmount);
        });
        ...
        it("应该正确计算攻击统计", async function () {
            const attackAmount = ethers.utils.parseEther("5.0");
    ...
    describe("整数溢出/下溢测试", function () {
        ...
        it("应该防止向零地址转账", async function () {
            await expect(
                overflowFixed.safeTransfer(ethers.constants.AddressZero, 100)
    ...
    describe("访问控制漏洞测试", function () {
        ...
        it("修复后的合约应该正确实施访问控制", async function () {
            ...
            // 所有者应该能调用
            await accessFixed.deposit({ value: ethers.utils.parseEther("10.0") });
        ...
        it("不能转移所有权给零地址", async function () {
            await expect(
                accessFixed.transferOwnership(ethers.constants.AddressZero)
    ...
    describe("前置交易攻击测试", function () {
        it("易受攻击的拍卖应该被前置交易攻击", async function () {
            const bid1 = ethers.utils.parseEther("5.0");
            const bid2 = ethers.utils.parseEther("6.0");
        ...
        it("提交-揭示模式应该防止前置交易攻击", async function () {
            const amount1 = ethers.utils.parseEther("5.0");
            const amount2 = ethers.utils.parseEther("6.0");
            const secret1 = ethers.utils.formatBytes32String("secret1");
            const secret2 = ethers.utils.formatBytes32String("secret2");

            // 生成盲做出价
            const blindedBid1 = ethers.utils.solidityKeccak256(
                ["address", "uint256", "bytes32"],
                [user1.address, amount1, secret1]
            );
            const blindedBid2 = ethers.utils.solidityKeccak256(
                ["address", "uint256", "bytes32"],
        ...
        it("应该拒绝无效的揭示", async function () {
            const amount = ethers.utils.parseEther("5.0");
            const secret = ethers.utils.formatBytes32String("secret");
            const wrongSecret = ethers.utils.formatBytes32String("wrong");
            const blindedBid = ethers.utils.solidityKeccak256(
                ["address", "uint256", "bytes32"],
                [user1.address, amount, secret]
            );
    ...
    describe("拒绝服务攻击测试", function () {
        it("易受攻击的分红分发应该被 DoS 攻击", async function () {
            // 添加多个投资者
            await dosVulnerable.connect(user1).invest({ value: ethers.utils.parseEther("10.0") });
            await dosVulnerable.connect(user2).invest({ value: ethers.utils.parseEther("20.0") });
            await dosVulnerable.connect(user3).invest({ value: ethers.utils.parseEther("15.0") });

            // 存入分红
            const dividendAmount = ethers.utils.parseEther("5.0");
        ...
        it("拉取支付模式应该防止 DoS 攻击", async function () {
            // 添加投资者
            await dosFixed.connect(user1).invest({ value: ethers.utils.parseEther("10.0") });
            await dosFixed.connect(user2).invest({ value: ethers.utils.parseEther("20.0") });
        ...
        it("应该正确计算分红", async function () {
            ...
            const dividendAmount = ethers.utils.parseEther("6.0");
            await dosFixed.depositDividends({ value: dividendAmount });
            ...
            expect(dividend2).to.be approximately(dividend1.mul(2), 1000);
        });

        it("批量退款应该只处理指定范围", async function () {
            await dosFixed.connect(user1).invest({ value: ethers.utils.parseEther("10.0") });
            await dosFixed.connect(user2).invest({ value: ethers.utils.parseEther("20.0") });
            await dosFixed.connect(user3).invest({ value: ethers.utils.parseEther("15.0") });
            ...
            expect(await dosFixed.balances(user3.address)).to.equal(ethers.utils.parseEther("15.0"));
        });
    ...
    describe("综合漏洞演示测试", function () {
        it("综合攻击应该能成功攻击易受攻击的金库", async function () {
            ...
            // 向金库发送资金
            await owner.sendTransaction({
                to: vulnerableVault.address,
                value: ethers.utils.parseEther("50.0")
            });

            const vaultBalanceBefore = await ethers.provider.getBalance(vulnerableVault.address);
            const attackerBalanceBefore = await ethers.provider.getBalance(comprehensiveAttacker.address);

            // 发起综合攻击
            await comprehensiveAttacker.connect(attackerAccount).reentrancyAttack({ value: attackAmount });

            const vaultBalanceAfter = await ethers.provider.getBalance(vulnerableVault.address);
            const attackerBalanceAfter = await ethers.provider.getBalance(comprehensiveAttacker.address);
        ...
        it("安全金库应该能防止所有攻击", async function () {
            ...
            await expect(
                secureVault.connect(user1).deposit({ value: ethers.utils.parseEther("1.0") })
        ...
        it("应该正确处理多个用户的存款和取款", async function () {
            await secureVault.connect(user1).deposit({ value: ethers.utils.parseEther("10.0") });
            await secureVault.connect(user2).deposit({ value: ethers.utils.parseEther("20.0") });
            await secureVault.connect(user3).deposit({ value: ethers.utils.parseEther("15.0") });

            expect(await secureVault.totalDeposits()).to.equal(ethers.utils.parseEther("45.0"));

            await secureVault.connect(user1).withdraw(ethers.utils.parseEther("5.0"));
            await secureVault.connect(user2).withdraw(ethers.utils.parseEther("10.0"));

            expect(await secureVault.totalDeposits()).to.equal(ethers.utils.parseEther("30.0"));
            expect(await secureVault.deposits(user1.address)).to.equal(ethers.utils.parseEther("5.0"));
            expect(await secureVault.deposits(user2.address)).to.equal(ethers.utils.parseEther("10.0"));
            expect(await secureVault.deposits(user3.address)).to.equal(ethers.utils.parseEther("15.0"));
        });
        ...
        it("应该正确记录攻击统计", async function () {
            const attackAmount = ethers.utils.parseEther("3.0");
    ...
    describe("安全对比测试", function () {
        ...
        it("应该展示不同安全措施的重要性", async function () {
            ...
            await secureVault.connect(user1).deposit({ value: ethers.utils.parseEther("5.0") });
            await secureVault.connect(user1).withdraw(ethers.utils.parseEther("2.0"));
            expect(await secureVault.deposits(user1.address)).to.equal(ethers.utils.parseEther("3.0"));
        });
    });
});
